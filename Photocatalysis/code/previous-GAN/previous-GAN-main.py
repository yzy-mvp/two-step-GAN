...
## For example, when running the first data sets, specify GAN as GAN0, and set data_index = 0, i.e. train_data_dict[0]
...
def Split_data(X_train, Y_train,seed, n_split=10):
    ...
    #A dictionary that stores training sets of different sizes, the keys are the the num of folds in subsets, and the values are the corresponding df data
...
def Target_featueres_split(data_xy,gen_data):
   
    max_scaler = MaxAbsScaler().fit(data_xy)
    gen_xy = max_scaler.inverse_transform(gen_data)
    
    X_gen = gen_xy[:,:-1]
    y_gen = gen_xy[:,-1]
    return X_gen,y_gen

def Generate_search(data_xy,train_data_scaled,train_x,train_y, X_test, Y_test, num):
    RMSE2_list = []
    for i in range(3):
        gan = GAN()
        ...
        #将生成数据分为特征和标签
        X_gen_fea,y_gen = Target_featueres_split(data_xy, gen_data)
...
if __name__ == '__main__':
    data = pd.read_csv(r'..\..\datasets\final_elem+hard-feat_data.csv')
    ...
    for seed in range(100,2101,500):
        ...
        train_x = train_data_dict[data_index][0]
        train_y = train_data_dict[data_index][1]
        #Adjust parameters based on the current training set
        ...
        num_list = [int(i*2) for i in (np.linspace(0.1,1.5,15) * len(train_data_dict[1][0]))]

        data_xy = np.concatenate((train_x, train_y.values.reshape(-1,1)),axis=1)
        train_data_scaled = MaxAbsScaler().fit_transform(data_xy)
        
        #Parallel computing to find the effect of different numbers of generated data
        pool = multiprocessing.Pool(5)
        results = pool.map(partial(Generate_search,data_xy,train_data_scaled,train_x,train_y, X_test, Y_test),num_list)
        pool.close()
        pool.join()
    ...
    print(rmse1_list)
    print('\n')
    print('The average of 5 divisions of rmse1 is:', mean_rmse1)
